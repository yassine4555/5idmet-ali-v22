...
async function main() {
    ...
    const institution = await InstitutionModel.findOneAndUpdate({ slug: 'demo-school' }, { $setOnInsert: { name: 'Demo School', type: institution_schema_1.InstitutionType.HIGH_SCHOOL, slug: 'demo-school', address: { city: 'Rabat', country: 'MA' }, settings: { academicYearStart: '2025-09-01', academicYearEnd: '2026-06-30', gradingSystem: '20', currency: 'MAD' }, isActive: true }, $set: { seededBy: seedTag } }, { upsert: true, new: true }).lean().exec();
    const institutionId = institution._id;
    console.log('Upserting admin user...');
    const adminEmail = 'admin@demo.school';
    const admin = await UserModel.findOneAndUpdate({ email: adminEmail }, { $setOnInsert: { institutionId, firstName: 'Admin', lastName: 'Demo', email: adminEmail, passwordHash: 'adminpass', role: user_schema_1.UserRole.SUPER_ADMIN, status: 'ACTIVE' }, $set: { seededBy: seedTag } }, { upsert: true, new: true }).lean().exec();
    const classNames = ['3ème A', '3ème B', '4ème A', '5ème C', '6ème A', 'Terminale S'];
    const classDocs = [];
    for (const n of classNames) {
        const c = await ClassGroupModel.findOneAndUpdate({ name: n, academicYear: '2025-2026' }, { $setOnInsert: { institutionId, name: n, level: n.split(' ')[0], academicYear: '2025-2026', studentIds: [] }, $set: { seededBy: seedTag } }, { upsert: true, new: true }).lean().exec();
        classDocs.push(c);
    }
    ...
    for (let i = 1; i <= teacherCount; i++) {
        ...
        const user = await UserModel.findOneAndUpdate({ email }, { $setOnInsert: { institutionId, firstName: `Teacher${i}`, lastName: 'Demo', email, passwordHash: `teachpass${i}`, role: user_schema_1.UserRole.TEACHER, status: 'ACTIVE' }, $set: { seededBy: seedTag } }, { upsert: true, new: true }).lean().exec();
        const profile = await TeacherProfileModel.findOneAndUpdate({ userId: user._id }, { $setOnInsert: { userId: user._id, institutionId, employeeNumber: `T-${i.toString().padStart(3, '0')}`, professionalInfo: { subjects: ['Math', 'Physics'].slice(0, (i % 2) + 1) }, personalInfo: {} }, $set: { seededBy: seedTag } }, { upsert: true, new: true }).lean().exec();
    ...
    for (let i = 1; i <= studentCount; i++) {
        ...
        const user = await UserModel.findOneAndUpdate({ email }, { $setOnInsert: { institutionId, firstName: `Student${i}`, lastName: 'Demo', email, passwordHash: `studpass${i}`, role: user_schema_1.UserRole.STUDENT, status: 'ACTIVE' }, $set: { seededBy: seedTag } }, { upsert: true, new: true }).lean().exec();
        const classToUse = classDocs[i % classDocs.length];
        const profile = await StudentProfileModel.findOneAndUpdate({ userId: user._id }, { $setOnInsert: { userId: user._id, institutionId, studentRegistrationId: `STU-${i.toString().padStart(4, '0')}`, academicInfo: { currentGradeLevel: classToUse.name }, financialInfo: { accountBalance: 0 } }, $set: { seededBy: seedTag } }, { upsert: true, new: true }).lean().exec();
    ...
    for (const [idx, s] of studentDocs.slice(0, 20).entries()) {
        for (let j = 0; j < 3; j++) {
            try {
                ...
                const payload = { institutionId, studentId: s.user._id, classId: s.classId, subject, type: 'Exam', score, maxScore: 20, teacherId: teacherDocs[j % teacherDocs.length].user._id, date, seededBy: seedTag };
                ...
                console.error('Grade upsert failed for student', s.user.email, 'subject', subjects[(j + idx) % subjects.length], 'error:', err.message);
            }
        }
    }
    ...
    for (let i = 0; i < 40; i++) {
        ...
        const payload = { institutionId, classId: classDoc._id, teacherId: teacher.user._id, subject: subjects[i % subjects.length], startTime: start, endTime: end, location: 'Room 101', seededBy: seedTag };
        try {
            ...
            console.error('Timetable upsert failed for class', classDoc.name || classDoc._id, 'teacher', teacher.user.email, 'error:', err.message);
        }
    }
    ...
    for (let i = 0; i < 10; i++) {
        try {
            ...
            const payload = { institutionId, invoiceNumber, studentId: s.user._id, items: [{ description: 'Tuition', amount: 2000 }], totalAmount: 2000, paidAmount: i % 3 === 0 ? 2000 : 0, status: i % 3 === 0 ? invoice_schema_1.InvoiceStatus.PAID : invoice_schema_1.InvoiceStatus.PENDING, dueDate: new Date('2026-09-30'), seededBy: seedTag };
...
//# sourceMappingURL=seed_tagged.js.map